...
struct GIOOctree
{
	...
	std::string getOctreeInfo()
    {
		std::stringstream ss;
    	ss << "\nPre-Shuffled (0=No, 1=Yes): " << preShuffled << std::endl;
    	ss << "Decomposition Level: " << decompositionLevel << std::endl;
    	ss << "Num Entries: " << numEntries << std::endl;
...
{
	// Returns an index of where each particle should be in O(n)

	// Determine the offset for each partition
	...
	// compute the position should be for the particle based on the leaf it's in
...
{ 
	// Reorders the particles of an array based on cyclic sort 

    // Fix all elements one by one 
    for (size_t i=0; i<n; i++) 
        while (index[i] != i) 	// While index[i] and arr[i] are not fixed 
        { 
			std::swap( arr[i], arr[index[i]] );
									...
									std::vector<int> partitionPosition, T array[], size_t numElements, bool shuffle)
{
  ...
  clock.start("create-index");
	std::vector<size_t> index = createIndex(numPartitions, partitionPosition, partitionCount, numElements);
  clock.stop("create-index");


  clock.start("reorder");
	reorder(array, &index[0], numElements);		// O(n) time and O(1) extra space
  ...
  clock.start("shuffle");
	if (shuffle)
	{
		// shuflle the particles in each partition
  ...
  clock.stop("overall");
    ...
    log << "Octree::reorganizeArrayInPlace create index took " << clock.getDuration("create-index") << " s " << std::endl;
    log << "Octree::reorganizeArrayInPlace reorder took " << clock.getDuration("reorder") << " s " << std::endl;
    log << "Octree::reorganizeArrayInPlace shuffle took " << clock.getDuration("shuffle") << " s " << std::endl;
  	log << "Octree::reorganizeArrayInPlace took " << clock.getDuration("overall") << " s " << std::endl << std::endl;
}
											 ...
											 int numLeaves, float leavesExtents[], std::vector<int> &leafPosition)
{
	...
	//leafCountMem.stop();
	clock.stop("overall");
	log << "Octree::findLeaf took " << clock.getDuration("overall") << " s " << std::endl;
...
inline std::vector<PartitionExtents> Octree::ComputeMyLeaves(float myRankExtents[6], int _numLevels)
{
	...
	clock.stop("overall");
	log << "Octree::chopVolume took " << clock.getDuration("overall") << " s " << std::endl;
